...
export default function ConcessionariasList() {
  ...
  return (
    <section className="conc-list">
      <div className="container">
        <div className="conc-list__filter">
          <span>FILTRO</span>
        ...
        <div className="conc-list__grid">
          {filtered.map((d) => (
            <Reveal as="div" className="conc-card" key={d.name}>
              <div className="conc-card__meta">
                <span className="conc-card__city">{d.city}</span>
                <span className="conc-card__state">{d.state}</span>
              </div>
              <h2 className="conc-card__name">{d.name}</h2>
              <p className="conc-card__address">{d.address}</p>
              {d.whatsapp && (
                <a
                  href={`https://wa.me/${d.whatsapp}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="conc-card__whatsapp"
                >
